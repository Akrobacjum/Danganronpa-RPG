...
19. **API socketu:** czy `game.socket` na v14 ma `listeners`, `off` i `on` - wtedy `game.drpg.relayGuard().state` jest `"ok"` (albo `"changed"`/`"unnamed"`), nie `"noApi"`, `"notFound"` ani `"backstop"`. `"ok"` nic nie mówi o `prependAny`, którego używa tylko tryb awaryjny: to osobny odczyt, `typeof game.socket.prependAny` w konsoli GM-a.
