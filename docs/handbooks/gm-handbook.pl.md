...
*Dla modułu "Danganronpa RPG" do Foundry VTT v14, wersja 1.2.62, zbudowanego na systemie Daggerheart.*
...
**Co żyje tylko w przeglądarkach GMów** (od 1.2.63): klucz odpowiedzi każdego Truth Bulletu; prawdziwy typ i odczyt każdego śladu; kto jest Mastermindem i gdzie jest jego kryjówka; obsada trwającego incydentu i Blackened każdego rozdziału; który podłożony przedmiot należy do której pułapki i co czeka w którym pokoju; oferowane Level Upy; które pokoje odkryła każda postać. Nic z tego nie jest danymi świata, więc konsola gracza tego nie przeczyta. Przeglądarka każdego GMa trzyma kopię dla każdego świata, a przeglądarki GMów wymieniają się kopiami, gdy któryś dołącza, pole po polu: wygrywa nowsza decyzja, usunięcie też, więc GM, który dołącza z pustą przeglądarką, niczego nie traci i niczego nie zabiera. Przeglądarka gracza trzyma tylko to, co wolno mu wiedzieć - czy sam jest Mastermindem, swoją rolę w incydencie, swoje oferty, pokoje swoich postaci. Deklaracje Observe czekające na rzut zostają tylko w przeglądarce głównego GMa.

**Kopia sprawy** (panel GMa, Między sesjami) zapisuje to wszystko do jednego pliku; trzymaj go tam, gdzie żaden gracz go nie otworzy. **Przywróć sprawę** scala plik z powrotem: dodaje tylko to, co nowsze, więc może to zrobić każdy GM, a dwa przywrócenia to jedno; plik innego świata i wiersze sprzed resetu sezonu zostają pominięte, chyba że zaznaczysz pole, które je bierze. Gdy przeglądarka głównego GMa otwiera świat z mniejszą wiedzą, niż świat mówi, że powinna mieć - nowa przeglądarka, wyczyszczone dane strony - kontrola zdrowia nazywa, czego brakuje, i proponuje **Przywróć z pliku** albo **Wpisz obsadę ręcznie** przy trwającym incydencie. Z konsoli raportują o tym `game.drpg.gmStoreStatus()` i `gmStoreHealth()`; `exportLedger()` i `importLedger()` to kopia i przywrócenie.
...
- **Kopia sprawy** (panel GMa), a plik tam, gdzie żaden gracz go nie otworzy: klucze odpowiedzi, ślady i Mastermind żyją tylko w przeglądarkach GMów.
