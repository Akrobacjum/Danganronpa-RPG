...
`scripts/voice.mjs`, the *Regional voice* setting plus LiveKit AVClient. Every mapped room becomes its own LiveKit breakout room; a player hears whoever is in the room with them, and their voice client follows their token. A Monokuma follows its own token, using the GM that its pool is mapped to. During an Eclipse everybody is in their own channel. As a GM you can **eavesdrop**: `game.drpg.voiceEavesdropDialog()` lists the rooms on the scene where your Monokuma stands and joins one as a muted listener - the people in that room see your tile in the camera dock, there is no hidden mode. **Reset all voice rooms** sends everybody back to the main room. `game.drpg.diagnoseVoice()` says which of the five links is the broken one; run it on the client that is complaining. A proximity-voice module installed alongside can silence a table while every check reports success - the diagnosis names it.
