...
`settings.mjs` and, when it lifts world data, a clause in `migrate.mjs` (R178 holds
that each lift is one, and that nothing else calls it) that reads back before it
removes anything (tier 2 runs the lifts that remove world data with their store's
save swallowed, and the world data stays); the claim, the reset's cuts, the exchange,
Back up and Restore follow from the row, and so do the health check's rows about a
claim; a store a player holds a copy of is named by the copy (`from`) and sends the
copies again after a restore (`afterRestore`, R182). A write whose value is derived -
a migration's default, an amendment - says so, `weak`, `fillOnly` or `ifLive` (R172),
or a second GM joining makes it the truth. Tier 2 holds the stores' sending while it
runs (`gmStoreHold`: its restore writes each key back on this client only) and stands
in a world the stores have never opened (`withGmStoreWorld`) for fixtures no other GM
may be sent. While either is on, no player is sent anything from a store - the door,
the cast, the fog, the offers (`gmStoresQuiet`) - a player's request for one waits
until it ends (`whenGmStoresAudible`) and is answered from the world this browser
opened, and the watches that tell the players a merged change keep what they were
last told until it ends and then hold the record against it: the primary tells what
another GM changed meanwhile (61 E6, F5), and what tier 2 put back raw compares equal
(R184; 61 J1b; 01 counts the packets tier 2 sends a player: none). `whenHydrated`
settles only on a real hydration - never on an open that threw - so a player's request
...
<!-- r-registry:start -->
Next free: R191. Reserved and unused: R113-R117, R120 for A5 (E13); R118-R119 for A5, written in E12; R121 for A4 (E22); R122 for A1 (E23); R123-R124 for A6 (E24).
...
| R165 | 1 | E31 | one wait: a request settles once, never rejects, and one message says what and why |
| R166 | 1 | E31 | a search is recorded on the scene its guard judged, not the one the packet names |
| R167 | 1 | E31 | handOff closes a window without waiting for its transition |
| R168 | 1 | E31 | a window's width counts its content's border once |
| R169 | 1 | E04 | the GM store's merge keeps every field that anybody wrote last |
| R170 | 1 | E04 | GM replicas converge by the protocol, and nothing from a player or another world is taken |
| R171 | 0 | E04 | nothing outside the engine reads or writes a GM store, a frozen legacy key or a player copy |
| R172 | 0 | E04 | no answer-key writer derives a value from absence |
| R173 | 1 | E04 | a restore never lowers a value, and a backup holds every store that says it is backed up |
| R174 | 1 | E04 | the case health report counts this world as it stands |
| R175 | 1 | E04 | a trace's question-mark icon is read, not written |
| R176 | 1 | E04 | a player keeps the newer of two stamped copies, and every copy the GM sends is stamped |
| R177 | 0 | E04 | the season reset is the primary's, and the clock cuts every store it wipes |
| R178 | 0 | E04 | no migration runs from a ready hook |
| R179 | 1 | E04 | a world that was in play keeps its safeword |
| R180 | 1 | E04 | a browser that takes over its old store after a season reset is cut by the reset |
| R181 | 1 | E04 | a restore takes no file's watermark, no other world's removal and no stamp beyond the clock |
| R182 | 1 | E04 | every store a player's copy is made from sends the copies again after a restore |
| R183 | 1 | E04 | a write of the cast stamps only the fields it names, and no turn is worked out from a browser with no cast |
| R184 | 1 | E04 | no player is told anything from a store while tier 2 holds the stores, and a request waits for it |
| R185 | 1 | E04 | the case is marked from the bullets and the traces alone, and the health window acts on what it showed |
| R186 | 1 | E04 | a failed save says who holds a copy, a crossed tab's write is written back, and a player's browser writes no GM store |
| R187 | 1 | E04 | an Analyze and a handover wait for the other GMs' copies before they call a key missing, and the case says whose old rows wait |
| R188 | 1 | E04 | the suite's first reading of the world waits for the load's own writes |
| R189 | 1 | E04 | an Analyze and a handover wait for the answer keys within a bound, and a GM store that cannot open says so |
| R190 | 1 | E05 | the world-secrets rule finds every secret planted in a world snapshot and nothing in a clean one |
